...
namespace TGZG {
    public class 自适应修正控制器 {
        ...
        public void Tick(double 差值, double 目标值速度, double 本帧时间) {
            //速度
            速度 = ((差值 - 上帧差值) / 本帧时间) - 目标值速度;
            //姿态状况
        ...
        double 目标回正路程用力比例 = 0.8;
        List<double> 最大加速度 = new() { 0 };
        double 回正修正(double 差值, double 本帧时间) {
            抖动和回正修正量 = 0;
            var 加速度 = (上帧速度 - 速度) / 本帧时间;
            if (上帧输出 is 1 && 上帧弹簧修正输出 is 1) {
                最大加速度.Add(加速度.绝对值());
            }
            if (上帧输出 is -1 && 上帧弹簧修正输出 is -1) {
                最大加速度.Add(加速度.绝对值());
            }
            $"上帧速度：{上帧速度:F2} 速度：{速度:F2} 加速度：{加速度:F2}".log();
            if (最大加速度.Count > 15) {
                最大加速度.RemoveAt(0);
            }
            double 目标路程 = 0;
            if (运动方向 is 运动方向.向上) {
                目标路程 = Math.Pow(速度, 2) / (最大加速度.Average() * 目标回正路程用力比例);
            } else {
                目标路程 = Math.Pow(速度, 2) / (最大加速度.Average() * 目标回正路程用力比例);
            }
            ...
            if (差值.绝对值() <= 目标路程.绝对值()) {
                ...
                //此处需要获取回正加速度的比例。在目标路程下，令速度减为0，所需要的回正力。
                var 所需回正加速度 = Math.Pow(速度, 2) / (2 * 差值.绝对值());
                var 回正力 = 所需回正加速度 / 最大加速度.Average();
                if (double.IsInfinity(回正力)) 回正力 = 0;
                //$"当前速度：{速度:F2} 最大加速度：{最大加速度.Average():F2} 所需回正加速度：{所需回正加速度:F2} ".Log();
                if (运动方向 is 运动方向.向上) {
                    抖动和回正修正量 = -回正力 - 其他阶段修正;
                } else {
                    抖动和回正修正量 = 回正力 - 其他阶段修正;
